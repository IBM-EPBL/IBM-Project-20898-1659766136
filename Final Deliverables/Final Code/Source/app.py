...
def detail(): 
    summary=[]
    if request.method=="POST":
      viewdata=request.form.get('view')
      print(viewdata)
      detailstatement="SELECT * FROM TicketDetails WHERE TICKETID=?"
      stmtd=ibm_db.prepare(conn,detailstatement)
      ibm_db.bind_param(stmtd,1,viewdata)
      ibm_db.execute(stmtd)
      summarydetails=ibm_db.fetch_both(stmtd)
      while summarydetails != False:
        summary.append(summarydetails)
        summarydetails = ibm_db.fetch_both(stmtd)
        
      return render_template("detail.html",details=summary) 
...
def assign():  
    ...
    agents=[]
    if request.method=="POST":
      resid=request.form.get('assign')
      statement="SELECT Username FROM Agent"
      stmt=ibm_db.prepare(conn,statement)
      ibm_db.execute(stmt)
      agent=ibm_db.fetch_both(stmt)
      while agent != False:
        agents.append(agent)
        agent = ibm_db.fetch_both(stmt)
      return render_template('assign.html',resid=resid,agents=agents) 

@app.route('/assignagent',methods =["POST"])   
def assign_agent():
    if request.method=="POST":
      addagent=request.form.get('aname')
      tid=request.form.get('tid')
      status="Agent Assigned"
      insert_sql="UPDATE TICKETDETAILS SET STATUS=?,AGENTNAME=? WHERE TICKETID=?"
      prep_stmt=ibm_db.prepare(conn,insert_sql)
      ibm_db.bind_param(prep_stmt,1,status)
      ibm_db.bind_param(prep_stmt,2,addagent)
      ibm_db.bind_param(prep_stmt,3,tid)
      ibm_db.execute(prep_stmt)
      return redirect(url_for('admindashboard'))
    return redirect(url_for('admindashboard'))
...
def resolution(): 
    global ticid
    if request.method=="POST":
      ticid=request.form.get('addre')
      print(ticid)
      return render_template('resolution.html',ticid=ticid) 
    return redirect(url_for('agentdashboard'))

@app.route('/addresolution',methods =["POST"])   
def add_resolution():    
    if request.method=="POST":
      resolutions=request.form.get('resolution')
      res=request.form.get('tid')
      print(resolution)
      status="Resolved"
      insert_sql="UPDATE TICKETDETAILS SET STATUS=?,RESOLUTION=? WHERE TICKETID=?"
      prep_stmt=ibm_db.prepare(conn,insert_sql)
      ibm_db.bind_param(prep_stmt,1,status)
      ibm_db.bind_param(prep_stmt,2,resolutions)
...
def deleteticket(): 
    if request.method=="POST":
     ...
     insert_sql="DELETE FROM TICKETDETAILS WHERE TICKETID=?"
     prep_stmt1=ibm_db.prepare(conn,insert_sql)
     ibm_db.bind_param(prep_stmt1,1,tid)
     ibm_db.execute(prep_stmt1)
     return redirect(url_for('customerdashboard'))
...
def viewcustomers(): 
    ...
    stmt=ibm_db.prepare(conn,statement)
    ibm_db.execute(stmt)
    customeracc=ibm_db.fetch_both(stmt)
    while customeracc != False:
        customers.append(customeracc)
        customeracc = ibm_db.fetch_both(stmt)
    return render_template('customeraccount.html',customers=customers) 

@app.route('/viewagents')    
def viewagents(): 
    agents=[]
    statement="SELECT * FROM Agent"
    stmt=ibm_db.prepare(conn,statement)
    ibm_db.execute(stmt)
    agentacc=ibm_db.fetch_both(stmt)
    while agentacc != False:
        agents.append(agentacc)
        agentacc = ibm_db.fetch_both(stmt) 
    return render_template('agentaccount.html',agents=agents) 


@app.route('/deleteagent',methods =["POST"])    
def deleteagent(): 
    if request.method=="POST":
     aid=request.form.get('remove') 
     insert_sql="DELETE FROM AGENT WHERE AGENTID=?"
     prep_stmt1=ibm_db.prepare(conn,insert_sql)
     ibm_db.bind_param(prep_stmt1,1,aid)
     ibm_db.execute(prep_stmt1)
     return redirect(url_for('viewagents'))
